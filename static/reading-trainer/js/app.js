...
registerMessages('ko', {
  ...
  'app.recovery.confirm2': '별도 복구 키와 내려받은 파일은 유지됩니다. 정말 새로 시작할까요?',
  ...
  'app.settings.reset_progress_confirm': '새 기록과 예전 훈련 기록을 초기화할까요? 내 글과 설정은 유지됩니다.',
  ...
  'app.settings.privacy_body': '앱은 계정이나 서버 저장을 쓰지 않습니다. 붙여넣은 글은 현재 브라우저에만 저장되고, 앱 화면에는 제3자 분석 자바스크립트가 없습니다.',
...
async function boot() {
  ...
  render();
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=20260713-36').catch(() => {});
}
