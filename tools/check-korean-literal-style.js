...
const rules = [
  ...
  },
  {
    ...
    pattern: /(?:(?:^|\n)계산재료과학과\s*머신러닝을\s*바탕으로,\s*원자|실험해\s*볼\s*소재\s*후보|전도성\s*코팅을\s*더해|여러\s*종류의\s*움직임과\s*신호를\s*구분할\s*수\s*있었고|큰\s*변형에서도\s*전기\s*신호가\s*안정적으로\s*유지됐습니다|미세구조의\s*형상[^.\n]{0,80}어떻게\s*바꾸는지\s*보였습니다|생체모사\s*구조를\s*공학\s*설계의\s*관점으로\s*풀어낸|자극\s*반응\s*전기신호를\s*오래\s*안정적으로\s*기록합니다|잔털처럼\s*가는\s*섬유|헤어리\s*파이버|백커버|표면에\s*미세한\s*털을\s*세운|어떻게\s*쓸\s*수\s*있는지를\s*보여준\s*연구|Authentic하게)/,
